...
//
// 			if (schematicsToLoad.Count == 0) schematicsToLoad = null;
// 		}
//
...
// 						{ "steamid64", steamid64 },
// 					};
// 					client.UploadValuesCompleted += GetSchematicsComplete;
...
// 		}
//
// 		private static MethodInfo LoadTilesMethodInfo;
// 		private static MethodInfo LoadWorldTilesVanillaMethodInfo;
//
...
// 				if (LoadTilesMethodInfo == null) LoadTilesMethodInfo = typeof(Main).Assembly.GetType("Terraria.ModLoader.IO.TileIO").GetMethod("LoadTiles", BindingFlags.Static | BindingFlags.NonPublic);
// 				if (LoadWorldTilesVanillaMethodInfo == null) LoadWorldTilesVanillaMethodInfo = typeof(Main).Assembly.GetType("Terraria.IO.WorldFile").GetMethod("LoadWorldTiles", BindingFlags.Static | BindingFlags.NonPublic);
// 				bool[] importance = new bool[TileID.Count];
// 				for (int i = 0; i < TileID.Count; i++) importance[i] = Main.tileFrameImportant[i];
//
...
// 				{
// 					LoadTilesMethodInfo.Invoke(null, new[] { tagCompound["m"] });
// 				}
//
...
// 				for (int i = 0; i < Main.maxTilesX; i++)
// 				for (int j = 0; j < Main.maxTilesY; j++)
// 					if (i < 6 || i >= Main.maxTilesX - 6 || j < 6 || j >= Main.maxTilesY - 6) loadedTilesExpanded[i, j] = new Tile();
// 					else loadedTilesExpanded[i, j] = Main.tile[i - 6, j - 6];
...
// 		}
//
// 		private static MethodInfo SaveTilesMethodInfo;
// 		private static MethodInfo SaveWorldTilesVanillaMethodInfo;
//
...
// 				Main.tile = tiles;
// 				if (SaveTilesMethodInfo == null) SaveTilesMethodInfo = typeof(Main).Assembly.GetType("Terraria.ModLoader.IO.TileIO").GetMethod("SaveTiles", BindingFlags.Static | BindingFlags.NonPublic);
// 				if (SaveWorldTilesVanillaMethodInfo == null) SaveWorldTilesVanillaMethodInfo = typeof(Main).Assembly.GetType("Terraria.IO.WorldFile").GetMethod("SaveWorldTiles", BindingFlags.Static | BindingFlags.NonPublic);
//
...
// 				}
//
// 				if (0 < list.Count) File.WriteAllLines(importPath, list);
...
// 			int texMaxX = (int)Math.Ceiling(maxX / (double)maxTile);
// 			int texMaxY = (int)Math.Ceiling(maxY / (double)maxTile);
...
// 								else
// 									textureTile = TextureAssets.Tile[tile.type];
//
...
// 								else
// 									textureWall = TextureAssets.Wall[tile.wall];
...
// 		// {
// 		// 	return tile != null && tile.color() > 0 && tile.color() < Main.numTileColors && Main.tileAltTextureDrawn[(int)tile.type, (int)tile.color()] && Main.tileAltTextureInit[(int)tile.type, (int)tile.color()];
// 		// }
//
